---
name: things-mac
description: "Управление Things 3 через `things` CLI на macOS (добавление/обновление проектов и задач через URL-схему; чтение/поиск/просмотр из локальной БД SQLite). Things database). Use when a user asks OpenClaw to add a task to Things, list inbox/today/upcoming, search tasks, or inspect projects/areas/tags."
