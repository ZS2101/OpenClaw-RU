---
name: gh-issues
description: "Получение задач (issues) GitHub, запуск субагентов для создания исправлений и открытия PR, затем мониторинг и обработка комментариев к PR. Использование: <issues> для ревью, <auto-fix> для запуска исправлений, <monitor> для отслеживания комментариев. /gh-issues [owner/repo] [--label bug] [--limit 5] [--milestone v1.0] [--assignee @me] [--fork user/repo] [--watch] [--interval 5] [--reviews-only] [--cron] [--dry-run] [--model glm-5] [--notify-channel -1002381931352]"
user-invocable: true
metadata:
  {
    "openclaw":
