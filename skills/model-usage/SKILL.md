---
name: model-usage
description: "Используйте CodexBar CLI для локального подсчета затрат и саммаризации использования моделей Codex или Claude, включая текущий (самый последний) расчетный месяц. model or a full model breakdown. Trigger when asked for model-level usage/cost data from codexbar, or when you need a scriptable per-model summary from codexbar cost JSON."
metadata:
  {
    "openclaw":
      {
