...
async function vtApiCall(
  cfg: OpenClawConfig, accountId: string, endpoint: string, params?: Record<string, any>,
): Promise<any> {
  const account = resolveVKTeamsAccount({ cfg, accountId: accountId ?? DEFAULT_ACCOUNT_ID });
  const token = resolveVTToken(account);
  if (!token) throw new Error("VK Teams token not configured");
  ...
  if (!res.ok) { const t = await res.text(); throw new Error(`VK Teams API error ${res.status}: ${t}`); }
  const data = await res.json();
  if (!data.ok) throw new Error(`VK Teams API: ${data.description ?? "unknown error"}`);
  return data;
}

// ─── Self ─────────────────────────────────────────────────

export async function getVKTeamsSelf(cfg: OpenClawConfig, accountId: string): Promise<any> {
...
export async function sendVKTeamsMessage(
  cfg: OpenClawConfig, accountId: string, chatId: string, text: string, replyMsgId?: string,
): Promise<{ msgId: string }> {
  const result = await vtApiCall(cfg, accountId, "/messages/sendText", { chatId, text, replyMsgId });
  return { msgId: result.msgId ?? "unknown" };
}

/** Send a file by reference (already uploaded file_id). */
export async function sendVKTeamsFile(
  cfg: OpenClawConfig, accountId: string, chatId: string, fileId: string, caption?: string,
): Promise<{ msgId: string }> {
...
export async function uploadVKTeamsFile(
  cfg: OpenClawConfig, accountId: string, chatId: string, buffer: Buffer, fileName: string, caption?: string,
  ...
  if (!res.ok) { const t = await res.text(); throw new Error(`VK Teams file upload error ${res.status}: ${t}`); }
  const data = await res.json();
  if (!data.ok) throw new Error(`VK Teams API: ${data.description ?? "unknown error"}`);
...
export async function editVKTeamsMessage(
  cfg: OpenClawConfig, accountId: string, chatId: string, msgId: string, text: string,
): Promise<void> {
  await vtApiCall(cfg, accountId, "/messages/editText", { chatId, msgId, text });
}

export async function deleteVKTeamsMessage(
  cfg: OpenClawConfig, accountId: string, chatId: string, msgId: string,
): Promise<void> {
...
export interface VTEvent {
  eventId: number; type: string;
  payload: {
    chat?: { chatId: string; type?: string; title?: string };
    from?: { userId: string; firstName?: string; lastName?: string; nick?: string; isBot?: boolean };
    text?: string;
    msgId?: string;
    timestamp?: number;
    parts?: Array<{ type: string; payload?: any }>;
  };
}

export async function getVTEvents(cfg: OpenClawConfig, accountId: string, lastEventId: number, pollTime = 60): Promise<{ events: VTEvent[] }> {
...
export async function getVTChatInfo(cfg: OpenClawConfig, accountId: string, chatId: string): Promise<any> {
  return vtApiCall(cfg, accountId, "/chats/getInfo", { chatId });
}

export async function sendVTAction(cfg: OpenClawConfig, accountId: string, chatId: string, action: string): Promise<void> {
  await vtApiCall(cfg, accountId, "/chats/sendActions", { chatId, actions: JSON.stringify([{ type: action }]) });
}

// ─── Probe ───────────────────────────────────────────────

export async function probeVKTeams(cfg: OpenClawConfig, accountId: string): Promise<VKTeamsProbeResult> {
  const account = resolveVKTeamsAccount({ cfg, accountId: accountId ?? DEFAULT_ACCOUNT_ID });
  const token = resolveVTToken(account);
  ...
  try {
    ...
    return { ok: true, latencyMs: Date.now() - startedAt, botName: info.firstName ?? info.nick ?? "VK Teams Bot" };
  } catch (err) {
