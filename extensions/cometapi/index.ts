...
export default definePluginEntry({
  id: "cometapi", name: "CometAPI Provider",
  ...
  register(api) {
    ...
    api.registerProvider({
      id: PROVIDER_ID, label: "CometAPI", docsPath: "/providers/cometapi", envVars: ["COMETAPI_API_KEY"],
      auth: [createProviderApiKeyAuthMethod({
        providerId: PROVIDER_ID, methodId: "api-key", label: "API-ключ CometAPI",
        hint: "API-ключ", optionKey: "cometapiApiKey", flagName: "--cometapi-api-key",
        envVar: "COMETAPI_API_KEY", promptMessage: "Введите API-ключ CometAPI",
        defaultModel: CometAPI_DEFAULT_MODEL_REF, expectedProviders: [PROVIDER_ID],
        applyConfig: (cfg) => applyCometAPIConfig(cfg),
        noteMessage: ["CometAPI — российский AI-прокси. OpenAI, Claude, 200+ моделей с оплатой в рублях.", "Любая модель: cometapi/gpt-4o, cometapi/claude-sonnet-4-5.", "Ключ: https://api.cometapi.com/console/token"].join("\n"),
        noteTitle: "CometAPI",
        wizard: { choiceId: "cometapi-api-key", choiceLabel: "API-ключ CometAPI", groupId: "cometapi", groupLabel: "₽ CometAPI", groupHint: "Российский AI-прокси" },
        groupSortKey: 1,
      })],
      catalog: { order: "simple", run: async (ctx) => {
        const k = ctx.resolveProviderApiKey(PROVIDER_ID).apiKey;
        if (!k) return null;
        // Seed the capabilities module with the API key so model lookups work
        setCometAPIApiKey(k);
        return { provider: { baseUrl: BASE_URL, api: "openai-completions", apiKey: k, models: [] } };
      }},
      staticCatalog: { order: "simple", run: async () => ({ provider: { baseUrl: BASE_URL, api: "openai-completions", models: [] } }) },
