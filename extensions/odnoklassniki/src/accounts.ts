...
export function resolveOKAccount(params: {
  ...
  const odnoklassnikiConfig = (params.cfg?.channels as any)?.odnoklassniki as OKConfig | undefined;
...
export function listOKAccountIds(cfg: OpenClawConfig): string[] {
  const odnoklassnikiConfig = (cfg?.channels as any)?.odnoklassniki as OKConfig | undefined;
  if (!odnoklassnikiConfig?.enabled && odnoklassnikiConfig?.enabled !== undefined) return [];
