...
export function resolveTamtamAccount(params: {
  ...
  const tamtamConfig = (params.cfg?.channels as any)?.tamtam as TamtamConfig | undefined;
...
export function listTamtamAccountIds(cfg: OpenClawConfig): string[] {
  const tamtamConfig = (cfg?.channels as any)?.tamtam as TamtamConfig | undefined;
  if (!tamtamConfig?.enabled && tamtamConfig?.enabled !== undefined) return [];
  const accounts = tamtamConfig?.accounts ?? {};
