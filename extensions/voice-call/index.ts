...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    fromNumber: { label: "Исходящий номер (From)", placeholder: "+15550001234" },
    toNumber: { label: "Номер получателя по умолчанию (Default To)", placeholder: "+15550001234" },
    inboundPolicy: { label: "Политика входящих (Inbound Policy)" },
    allowFrom: { label: "Белый список входящих (Allowlist)" },
    inboundGreeting: { label: "Приветствие для входящих", advanced: true },
    "telnyx.apiKey": { label: "API-ключ Telnyx", sensitive: true },
    "telnyx.connectionId": { label: "Connection ID Telnyx" },
    "telnyx.publicKey": { label: "Публичный ключ Telnyx", sensitive: true },
    "twilio.accountSid": { label: "Account SID Twilio" },
    "twilio.authToken": { label: "Auth Token Twilio", sensitive: true },
    "outbound.defaultMode": { label: "Режим звонка по умолчанию" },
    "outbound.notifyHangupDelaySec": {
      label: "Задержка уведомления о завершении (сек)",
      advanced: true,
    },
    "serve.port": { label: "Порт вебхука" },
    "serve.bind": { label: "Привязка (bind) вебхука" },
    "serve.path": { label: "Путь (path) вебхука" },
    "tailscale.mode": { label: "Режим Tailscale", advanced: true },
    "tailscale.path": { label: "Путь (path) Tailscale", advanced: true },
    "tunnel.provider": { label: "Провайдер туннеля", advanced: true },
    "tunnel.ngrokAuthToken": {
      label: "Auth Token ngrok",
      sensitive: true,
      advanced: true,
    },
    "tunnel.ngrokDomain": { label: "Домен ngrok", advanced: true },
    "tunnel.allowNgrokFreeTierLoopbackBypass": {
      label: "Разрешить бесплатный тариф ngrok (обход Loopback)",
      advanced: true,
    },
    "streaming.enabled": { label: "Включить стриминг", advanced: true },
    "streaming.provider": {
      label: "Провайдер стриминга",
      help: "Uses the first registered realtime transcription provider when unset.",
      advanced: true,
    },
    "streaming.providers": { label: "Конфиг провайдера стриминга", advanced: true },
    "streaming.streamPath": { label: "Путь медиа-потока", advanced: true },
    "realtime.enabled": { label: "Включить Realtime Voice", advanced: true },
    "realtime.provider": {
      label: "Провайдер Realtime Voice",
      help: "Uses the first registered realtime voice provider when unset.",
      advanced: true,
    },
    "realtime.streamPath": { label: "Путь Realtime-потока", advanced: true },
    "realtime.instructions": { label: "Инструкции для Realtime", advanced: true },
    "realtime.providers": { label: "Конфиг провайдера Realtime", advanced: true },
    "tts.provider": {
      label: "Переопределение провайдера TTS",
      help: "Deep-merges with messages.tts (Microsoft is ignored for calls).",
      advanced: true,
    },
    "tts.providers": { label: "Конфиг провайдера TTS", advanced: true },
    publicUrl: { label: "Публичный URL вебхука", advanced: true },
    skipSignatureVerification: {
      label: "Пропустить проверку подписи",
      advanced: true,
    },
    store: { label: "Путь хранения логов звонков", advanced: true },
    responseModel: {
      label: "Модель для ответа",
      help: "Optional override. Falls back to the runtime default model when unset.",
      advanced: true,
    },
    responseSystemPrompt: { label: "Системный промпт для ответа", advanced: true },
    responseTimeoutMs: { label: "Тайм-аут ответа (мс)", advanced: true },
  },
};
...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool({
      name: "voice_call",
      label: "Голосовой звонок (Voice Call)",
