...
export default definePluginEntry({
  id: "selectel", name: "Selectel Foundation Models",
  description: "Встроенный провайдер Selectel с настраиваемым endpoint",
  register(api) {
    api.registerProvider({
      id: "selectel", label: "Selectel", docsPath: "/providers/selectel", envVars: ["SELECTEL_API_KEY", "SELECTEL_BASE_URL"],
      auth: [
        createProviderApiKeyAuthMethod({
          providerId: "selectel", methodId: "api-key", label: "API-ключ Selectel",
          hint: "Ключ сервиса инференса", optionKey: "selectelApiKey", flagName: "--selectel-api-key",
          envVar: "SELECTEL_API_KEY", promptMessage: "Введите API-ключ Selectel",
          defaultModel: Selectel_DEFAULT_MODEL_REF, expectedProviders: ["selectel"],
          ...
          noteTitle: "Selectel",
          wizard: { choiceId: "selectel-api-key", choiceLabel: "API-ключ Selectel", groupId: "selectel", groupLabel: "₽ Selectel", groupHint: "Ключ Foundation Models" },
          groupSortKey: 1,
        }),
        {
          id: "base-url",
          ...
          wizard: { choiceId: "selectel-base-url", choiceLabel: "URL эндпоинта Selectel", groupId: "selectel", groupLabel: "₽ Selectel", groupHint: "Endpoint сервиса инференса" },
          groupSortKey: 1,
          run: async (ctx): Promise<{ profiles: Array<{ profileId: string; credential: Record<string, unknown> }>; configPatch: Partial<OpenClawConfig>; notes: string[] }> => {
            const baseUrl = await ctx.prompter.text({
              message: "Введите URL endpoint сервиса инференса Selectel (например, https://xxx.selectel.ru/v1):",
              placeholder: "https://xxx.selectel.ru/v1",
              validate: (v: string) => {
                const trimmed = v.trim();
                if (!trimmed) return "URL endpoint обязателен";
                if (!/^https?:\/\/.+/.test(trimmed)) return "URL must start with https://";
                return undefined;
              },
            });
            ...
            return {
              profiles: [],
              configPatch: { env: { vars: { SELECTEL_BASE_URL: trimmed } } } as Partial<OpenClawConfig>,
              notes: [
      ...
      catalog: { order: "simple", run: async (ctx) => {
        ...
        return { provider: { baseUrl: baseUrl || undefined, api: "openai-completions", apiKey: k, models: [] } };
      }},
      staticCatalog: { order: "simple", run: async () => {
        const baseUrl = getSelectelBaseUrl();
        return { provider: { baseUrl: baseUrl || undefined, api: "openai-completions", models: [] } };
      }},
      resolveDynamicModel: (ctx): ProviderRuntimeModel => {
        const baseUrl = getSelectelBaseUrl();
        return {
          id: ctx.modelId, name: ctx.modelId, api: "openai-completions",
          provider: "selectel", baseUrl: baseUrl || undefined, reasoning: false,
