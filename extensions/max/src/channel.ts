...
const maxOutboundAdapter = {
  sendPayload: async (params: {
    ...
    const result = await sendMessageMAX(params.cfg, params.accountId, {
      target: normalizeMAXMessagingTarget(params.to),
...
export const maxPlugin = createChatChannelPlugin({
  ...
  lifecycle: {
    onStart: async (params) => {
      ...
      const stop = await webhook.startMAXMonitor({
        cfg: params.cfg,
      ...
      return {
        stop: () => {
          webhook.setMAXWebhookHandler(null);
          stop();
        },
      };
    },
  ...
  gateway: {
    registerHttpHandlers: ({ app }) => {
      app.post("/webhooks/max", async (req: Request, res: Response) => {
        try {
          const webhook = await loadWebhook();
          await webhook.handleMAXWebhookEvent(req.body);
          res.json({ ok: true });
        } catch (err) {
  ...
  outbound: {
    resolveRoute: ({ cfg, accountId, to }) =>
      buildChannelOutboundSessionRoute({ cfg, channel: "max", accountId, to }),
    send: async (params) => {
      const result = await maxOutboundAdapter.sendPayload(params);
      return attachChannelToResult(result, { channel: "max" });
    },
  },
  ...
  status: {
    ...
    resolveConfigured: ({ cfg, accountId }) =>
      resolveConfiguredFromCredentialStatuses({
        ...
        resolveCredentialStatus: () => {
          const token = resolveMAXToken(resolveMAXAccount({ cfg, accountId }));
    ...
    buildSummary: buildTokenChannelStatusSummary({
      channel: "max",
      label: "MAX",
      formatAccountLabel: (a) => a.name ?? "MAX",
    }),
  },
  ...
  security: {
    checkDmAccess: ({ cfg, accountId, senderId }) => {
      ...
      if (policy === "allowlist") {
        return {
          allowed:
            (account.allowFrom ?? []).includes("*") || (account.allowFrom ?? []).includes(senderId),
        };
      }
      return { allowed: true };
    },
  ...
  groups: {
    resolveRequireMention: ({ cfg, accountId, groupId }) =>
      resolveMAXAccount({ cfg, accountId }).groups?.[groupId]?.requireMention ?? false,
  },
  ...
  directory: createChannelDirectoryAdapter({
    ...
    resolveSelf: async ({ cfg, accountId }) => {
      const account = resolveMAXAccount({ cfg, accountId });
      try {
        const info = await (await import("./api.js")).probeMAX(cfg, accountId);
        return { id: "self", name: info.botName ?? "MAX Bot", type: "bot" };
      } catch {
        return { id: "self", name: account.name ?? "MAX Bot", type: "bot" };
      }
    },
    resolvePeers: async () => [],
    resolveGroups: async ({ cfg, accountId }) =>
      Object.entries(resolveMAXAccount({ cfg, accountId }).groups ?? {}).map(([id]) => ({
        id,
        name: `Chat ${id}`,
        configured: true,
      })),
  ...
  messaging: {
    ...
    looksLikeTargetId: looksLikeMAXTargetId,
  },
});
