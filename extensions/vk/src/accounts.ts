...
export function resolveVkAccount(params: {
  ...
  const vkConfig = (params.cfg?.channels as unknown as Record<string, unknown> | undefined)?.vk as
    | VkConfig
    | undefined;
...
export function listVkAccountIds(cfg: OpenClawConfig): string[] {
  const vkConfig = (cfg?.channels as unknown as Record<string, unknown> | undefined)?.vk as
    | VkConfig
    | undefined;
  if (!vkConfig?.enabled && vkConfig?.enabled !== undefined) {
    return [];
  }
